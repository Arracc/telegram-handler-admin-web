    ...
    render() {
        return (
            <Layout className='animated fadeIn'>
                    ...
                    <Col>
                        <div className='base-style'>
                            ...
                            <GroupTable
                                ...
                                showGroupInfoModal={data => this.showGroupInfoModal(data)}
                                showGroupMemberTable={data => this.showGroupMemberTable(data)}
                                showGroupMessageTable={data => this.showGroupMessageTable(data)}
                            />
                        </div>
                    </Col>
                </Row>
                ...
                <GroupMessageTableModal
                    groupId={this.state.groupId}
                    ref={node => (this.GroupMessageTableModal = node)}
                />
            </Layout>
        )
    }
    ...
    render() {
        return (
            <Table
                ...
                dataSource={this.state.data.groupList}
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record, index) => {
                return (
                    <span>
                        <Button type='button' onClick={() => this.showGroupInfo(record.groupId)}>
                            查看群信息
                        </Button>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <Button type='button' onClick={() => this.showGroupMemberList(record.groupId)}>
                            查看群成员
                        </Button>
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <Button type='button' onClick={() => this.showGroupMessageList(record.groupId)}>
                            查看群消息
                        </Button>
                    </span>
                )
...
class GroupInfoModal extends Component {
    ...
    getGroupInfo = params => {
        ...
        axios
            .post(url, { groupId: params.groupId })
            .then(res => {
                if (res.data.code === 200) {
                    const pagination = { ...this.state.pagination }
                    pagination.total = 200
    ...
    columns = [
        ...
        },
        {
            ...
            render: (text, record, index) => {
                return (
                    <span>
                        <Button type='button'>查看用户信息</Button>
                    </span>
                )
            }
        }
